...
class DiscretizerNP:
    """
    PHASE 2 CHANGES:
    - timestep = 1.0 (matching authors)
    - append_masks = True by default
    ...
    def transform(self, hours: np.ndarray, values: np.ndarray) -> np.ndarray:
        """
        Discretize time series with proper forward-fill imputation.
        
        Args:
            hours: [N,] array of timestamps
            values: [N, F] array of feature values
            
        Returns:
            [T, F] or [T, 2F] array (if append_masks=True)
        """
        if values.size == 0:
        ...
        # Initialize data and mask matrices
        X = np.zeros((T_bins, F), dtype=np.float32)
        M = np.zeros((T_bins, F), dtype=np.float32)
        
        # Map measurements to bins (last observation per bin)
        ...
        for i, b in enumerate(bin_idx):
            ...
            if not finite.any():
                continue
            # Take last observation in each bin
            X[b, finite] = row[finite].astype(np.float32)
            M[b, finite] = 1.0

        # CRITICAL: Implement forward-fill imputation (authors' 'previous' strategy)
        if self.impute_strategy == 'previous':
            for f in range(F):
                last_val = 0.0  # Initial value for features with no early measurements
                for t in range(T_bins):
                    if M[t, f] == 1.0:
                        # We have a measurement, update last_val
                        last_val = X[t, f]
                    else:
                        # No measurement, forward-fill from last observation
                        X[t, f] = last_val
        elif self.impute_strategy == 'normal':
            # Use normal/typical values (simplified - use 0 for now)
            # In authors' code, they have a lookup table of normal values per feature
            for f in range(F):
                for t in range(T_bins):
                    if M[t, f] == 0.0:
                        X[t, f] = 0.0
        # else: 'zero' strategy - already initialized to zero

        # CRITICAL: Append masks as additional features (authors' store_masks=True)
...
class NormalizerNP:
    """
    PHASE 2: Properly handles continuous vs categorical features.
    Standardize only VALUE columns (first F features). Mask columns remain 0 or 1.
    Stores n_value_feats so we always skip masks correctly.
    
    IMPORTANT: If append_masks=True, the data will have shape [T, 2F] where:
    - X[:, :F] are the values (to be normalized)
    - X[:, F:2F] are the masks (should NOT be normalized)
    """
    ...
    def fit(self, matrices: List[np.ndarray], n_value_feats: int):
        """
        Fit normalizer on VALUE features only.
        
        Args:
            matrices: List of [T, F] or [T, 2F] arrays
            n_value_feats: Number of value features (F, not 2F)
        """
        self.n_value_feats = int(n_value_feats)
        Fv = self.n_value_feats
        
        # Accumulate statistics only on value columns
        s = np.zeros(Fv, dtype=np.float64)
        ss = np.zeros(Fv, dtype=np.float64)
        n = np.zeros(Fv, dtype=np.int64)
        
        for X in matrices:
            # Only use first Fv columns (values, not masks)
            Xv = X[:, :Fv]
    ...
    def transform(self, X: np.ndarray) -> np.ndarray:
        """
        Normalize only the VALUE columns, leave masks unchanged.
        ...
        # Only normalize value columns
        X_out[:, :Fv] = (X[:, :Fv] - self.means[:Fv]) / self.stds[:Fv]
        
        # Mask columns (if present) remain unchanged
        return X_out
...
def materialize_split(
    split: str,
    timestep: float = 1.0,  # PHASE 2: Changed from 0.8 to 1.0
    append_masks: bool = True,  # PHASE 2: Changed from False to True
    impute_strategy: str = 'previous',  # PHASE 2: Added forward-fill
):
    """
    ...
    PHASE 2 CHANGES:
    - timestep = 1.0 (matching authors' arg_timestep)
    - append_masks = True (authors' store_masks=True)
    - impute_strategy = 'previous' (authors' forward-fill)
    """
    ...
    for i, (ts_path, y) in enumerate(reader.iter_samples()):
        ...
        # Determine number of value features (excluding masks)
        if n_value_feats is None:
            n_value_feats = X.shape[1] // 2 if discret.append_masks else X.shape[1]
            print(f"[INFO] Detected {n_value_feats} value features, total width={X.shape[1]}")
    ...
    print(f"     Expected input_dim = {X_list[0].shape[1]}\n")


if __name__ == "__main__":
    print("\n" + "="*60)
    print("PHASE 2: DATA PREPROCESSING FIXES")
    print("="*60)
    print("Changes applied:")
    print("  1. timestep: 0.8 → 1.0 (matching authors)")
    print("  2. append_masks: False → True")
    print("  3. impute_strategy: none → 'previous' (forward-fill)")
    print("="*60 + "\n")
