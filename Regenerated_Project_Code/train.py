    ...
    for X, D, y in loader:
        ...
        if scaler:
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                y_prob, decov = model(X, D)
                loss = torch.nn.functional.binary_cross_entropy(y_prob, y) + lambda_decov * decov
...
def _save_curves(y_true, y_prob, results_dir: str):
    """Save ROC and PR curves to results_dir."""
...
def main():
    ...
    # Match the paper seed and enable strong determinism
    set_seed(42)
