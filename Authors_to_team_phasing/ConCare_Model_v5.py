...
class SingleAttentionPerFeatureNew(nn.Module):
    ...
    def __init__(self, hidden_dim, attention_hidden_dim=8):
        ...
        # CHANGE #1: initialize to 0.8 to match base
    ...
    def forward(self, H):
        ...
        # CHANGE #2: fixed 48-step time index like the base code
        # Assumes sequences are length 48
        b_time = torch.arange(47, -1, -1, dtype=torch.float32, device=device).unsqueeze(0).repeat(B, 1) + 1.0

        # Authors' nonlinear time-aware normalization
        denom = self.sigmoid(self.rate) * (torch.log(2.72 + (1 - self.sigmoid(dot_product))) * b_time)

        # Scores -> weights
        e = self.relu(self.sigmoid(dot_product) / denom)      # [B, T]
...
class ConCare(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, d_model, MHD_num_head, d_ff, output_dim, keep_prob, demographic_dim):
        ...
        self.demo_proj = nn.Linear(self.demographic_dim, self.hidden_dim)  # parity with authors

        # CHANGE #3: tie MHA dropout to 1 - keep_prob to match base behavior
        self.MultiHeadedAttention = MultiHeadedAttention(self.d_model, self.MHD_num_head, dropout=1 - self.keep_prob)
