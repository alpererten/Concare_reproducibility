...
class SingleAttention(nn.Module):
    ...
    def forward(self, input, demo=None):
        ...
        time_decays = torch.arange(time_step - 1, -1, -1, dtype=torch.float32, device=device).unsqueeze(0).unsqueeze(-1)  # 1*t*1
...
class ConCare(nn.Module):
    def __init__(self, input_dim, hidden_dim, d_model,  MHD_num_head, d_ff, output_dim, keep_prob=0.5, demographic_dim = 12):
